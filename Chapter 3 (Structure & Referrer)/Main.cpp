...
*  반환타입 함수이름 (타입 이름, ...) {}
* 
* 1. 반환타입 - 참조자로 사용되는 경우, 아닌경우
* 2. 매개변수 - 참조자로 사용되는 경우, 아닌경우
*/

/* 이번 수업에서 배울것
*  함수 오버로딩(Overloading)
*  플레이어 x,y좌표를 입력받아서 좌표를 변경시키는 함수
*  플레이어의 좌표 구조체를 입력받아서 좌표를 변경시키는 함수
*  같은 이름 타입만 다르게 받으면 컴퓨터(컴파일러)가 구분할 수 있다.
*/

// C++ 부터는 typedef 선언이 없어도, struct 이름으로 타입과 구조체를 동시에 사용할 수 있다.
struct Pos
{
	int x;
	int y;
...
* Ex)
* void PlayerInput(), int PlayerInput() 오버로드 할 수 없다.
